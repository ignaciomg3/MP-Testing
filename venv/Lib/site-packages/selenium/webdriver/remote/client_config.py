...
class ClientConfig:
    ...
    def proxy(self) -> Proxy:
        """:Returns: The proxy used for communicating to the drivers/server."""
        return self._proxy

    @proxy.setter
    def proxy(self, proxy: Proxy) -> None:
        """Provides the information for communicating with the drivers or
        server.
        For example: Proxy(raw={"proxyType": ProxyType.SYSTEM})

        :Args:
         - value: the proxy information to use to communicate with the drivers or server
        """
        self._proxy = proxy
    ...
    def timeout(self, timeout: int) -> None:
        """Provides the timeout (in seconds) for communicating with the drivers
        or server.

        :Args:
         - value: the timeout (in seconds) to use to communicate with the drivers or server
        """
