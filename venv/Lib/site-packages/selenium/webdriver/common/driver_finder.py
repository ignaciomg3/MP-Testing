...
class DriverFinder:
    """A Driver finding class responsible for obtaining the correct drivers and
    associated browser.

    :param service: instance of the drivers service class.
    ...
    def _binary_paths(self) -> dict:
        ...
        try:
            ...
            if path:
                logger.debug(
                    "Skipping Selenium Manager; path to %s drivers specified in Service class: %s", browser, path
                )
                ...
                if Path(output["driver_path"]).is_file():
                    ...
                    raise ValueError(f'The drivers path is not a valid file: {output["driver_path"]}')
            ...
            msg = f"Unable to obtain drivers for {browser}"
